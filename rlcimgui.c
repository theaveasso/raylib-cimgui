                ...
                if (!(io->ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange))
                {
                    SetMouseCursor(
                        (imgui_cursor > -1 && imgui_cursor < ImGuiMouseCursor_COUNT)
                            ? ImGuiCursorToRaylib(imgui_cursor)
...
void rligSetupFontAwesome(void) {
    ...
    ImFontAtlas_AddFontFromMemoryCompressedTTF(
        io->Fonts, (void *) fa_solid_900_compressed_data,
...
void rligImageRect(const Texture *image, int destWidth, int destHeight,
...
void rligImageRenderTexture(const RenderTexture *image) {
    ...
    rligImageRect(&image->texture, image->texture.width, image->texture.height,
